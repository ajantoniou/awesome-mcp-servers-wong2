...
- **[UploadCheck](https://github.com/ajantoniou/uploadcheck-mcp)** - Pre-publish media quality gate: runs 40+ deterministic checks on a finished video/audio export (frozen frames, dead air, lip-sync drift, loudness, caption safe-area, temporal warping) and returns PASS/WATCH/BLOCK with timestamped fixes, so an agent repairs only the flagged span and re-checks before publishing.
